...
PERCENTAGE = .8 #Split train/val and test set
CALLBACK = .031 #Used to stop training the Network when the MAE from the validation set reached a perormance below 3.1%
BATCH_SIZE = 10 #Number of samples that will be propagated through the network. I chose almost a trading month
EPOCH = 30 #Settled to train the model
WINDOW_LSTM = 30 #The window used for the input data
PREDICTION_SCOPE = 1 #How many period to predict, being 0=1
...
print(train_scale_lstm)
print(val_scale_lstm)
...
mae_lstm = mean_absolute_error(real_val.iloc[:, 48], pred_val.iloc[:, 48])
...
plt.plot(real_val.iloc[:, 5])
plt.plot(pred_val.iloc[:, 5])
...
plt.plot(real_train.iloc[4000:, 48])
plt.plot(pred_train.iloc[4000:, 48])
...
y_val_lstm = np.array(real_val.iloc[-30:, 48])
y_test_lstm = np.array(real_test.iloc[:, 48])
pred_test = np.array(pred_test.iloc[-1:, 48])
mae_lstm = mean_absolute_error(real_val.iloc[:, 48], pred_val.iloc[:, 48])
...
plotting(y_val_save_LG, y_test, scope, avg_mae, WINDOW, PREDICTION_SCOPE)
